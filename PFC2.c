...
int main(int argc, char *argv[]){
	...
	int fd, charsRead, fdPipe, nWrite;
	...
	if( fd < 0){	//se apertura fallisce
		printf("Errore apertura file G18 da PFC2\n");
	}

	//apro pipe, fallisce se non ho aperto il lato lettura
	do{
		fdPipe = open("pipePFC2", O_WRONLY);
		if(fdPipe == -1){	//riprovo tra 5ms
			usleep(5*1000);
		}
	}while(fdPipe == -1);
	...
	do{
		...
		do{
			...
			if(strcmp(buffer, gpgll) == 0){	//se ho letto $GPGLL,
				...
				//invia la speed tramite pipe
				sprintf(tmp, "%f", speed);
				fflush(NULL);	//puntatore a NULL la scrittura avviene per tutti i files aperti in output
				nWrite = write(fdPipe, tmp, strlen(tmp));
				fflush(NULL);	//puntatore a NULL la scrittura avviene per tutti i files aperti in output
				if(nWrite == -1){
					printf("Errore scrittura pipe da PFC2\n");
				}
				do{
