...
int main(int argc, char *argv[]){
	...
	int fd, charsRead, fdSpeed, charsWritten;
	...
	if( fd < 0){	//se apertura fallisce
		printf("Errore apertura file G18 da PFC3\n");
	}

	//apro speed.txt, e' il file condiviso con transducers
	fdSpeed = open("speed.txt", O_WRONLY|O_CREAT, 0666);
	if( fdSpeed < 0){
		printf("Errore apertura file speed da PFC3");
	}
	...
	do{
		...
		bool find = false;
		do{
			charsRead = read(fd,buffer,7); //leggo primi 7 byte per capire in che riga sono
			if(strcmp(buffer, gpgll) == 0){	//se ho letto $GPGLL,
				//calcolo la velocita'
				speed = computeSpeed(fd, &longitudeOld, &latitudeOld);
				//invia la speed tramite file
				sprintf(tmp, "%f\n", speed);
				fflush(NULL);
				charsWritten = write(fdSpeed, tmp, strlen(tmp));
				fflush(NULL);
				if(charsWritten == -1){
					printf("Errore scrittura file da PFC3\n");
				}
				do{
